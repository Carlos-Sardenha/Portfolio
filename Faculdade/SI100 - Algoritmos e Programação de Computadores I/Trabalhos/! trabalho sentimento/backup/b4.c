...
void encontrarMaiorPalavra(char positivas_separadas[][21], int numLinhas) {
    char maiorPalavra[21];
    int i;

    // Inicializar com a primeira palavra
    strcpy(maiorPalavra, positivas_separadas[0]);

    // Encontrar a maior palavra
    for (i = 1; i < numLinhas; i++) {
        if (strlen(positivas_separadas[i]) > strlen(maiorPalavra)) {
            strcpy(maiorPalavra, positivas_separadas[i]);
        }
    }
...
int main()
{
    ...
    if (questoes[0][0] > 0)
    {
        ...
        else if (questoes[0][0] == 3)
        {
            ...
            encontrarMaiorPalavra(positivas_separadas, linhas);
        }
