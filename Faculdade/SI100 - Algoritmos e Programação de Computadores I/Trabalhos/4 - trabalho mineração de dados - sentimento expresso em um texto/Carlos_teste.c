...
//-->Criar sentença
void criar_sentenca(char frases[][linhas], int colunas, char sentences[][linhas]) {
    int sentence_count = 0;

    for (int i = 0; i < colunas; i++) {
        char *token = strtok(paragraphs[i], ".!?"); // Separa o parágrafo em sentenças

        while (token != NULL) {
            strcpy(sentences[sentence_count], token); // Copia a sentença
            token = strtok(NULL, ".!?"); // Próxima sentença
            sentence_count++;
        }
    }

    return sentence_count; // Retorna o número total de sentenças
}

//--> Imprimir maior string positiva
void polaridade_sentenca_mais_longa(){}
...
int main()
{
    ...
    leitura_palavras_positivas_negativas(palavras_positivas, palavras_negativas, positivas_separadas, negativas_separadas);//--> Faz a leitura da quantidade de positivas e negativas
    ...
    if (questoes[0][0] > 0)
    {
        ...
        {
            int linhas, colunas;
            linhas = sizeof(positivas_separadas) / sizeof(positivas_separadas[0]); // Tamanho total da matriz dividido pelo tamanho da primeira linha
            colunas = sizeof(positivas_separadas[0]) / sizeof(positivas_separadas[0][0]); // Tamanho da primeira linha dividido pelo tamanho de um elemento
            char criar_sentenca(paragrafos, colunas, frases);
            //criar_sentenca(char frases[][linhas], int colunas, char sentences[][linhas])
        }
