...
int main()
{
    ...
    int positivas_count;//--> Quantidade palavras positivas
    int negativas_count;//--> Quantidade palavras positivas
    ...
    {
        fgets(paragrafos[i][0], 2000, stdin);
    }
    ...
    for (int i = 0; i < num_questoes; i++)//--> Recolhe questoes a serem respondidas
    {
        ...
        if (questoes[i][1] == 2)//--> Se for a questao 2, recolhe o paragrafo a ser respondido
        {
    ...
    // -->Questão 1
    if(questoes[0][0] < 0){//Se houver questao na posicao, então descobrir qual a questao a responder
        if(questoes[0][0] == 1)
        {
